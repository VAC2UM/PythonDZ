def parse_data(text):
    ...
    pairs = [pair.strip().split('@') for pair in data_section.split('set') if pair.strip()]
    result = [(name.strip(), value.strip()) for name, value in pairs]
    cleaned_data = [(name, value.strip(".'")) for name, value in result]  # Removed eval() here
    return cleaned_data


input_string = "begin set getiri @'soed_92'. set arla_890 @'quxe'. set erle_404 @'are'. end"
print(parse_data(input_string))
